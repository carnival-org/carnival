import pytest
from carnival import cmd

KEY = "ssh-rsa  example@laptop"  # noqa


@pytest.mark.remote
def test_ssh_authorized_keys_list(suspend_capture, ubuntu_ssh_host, centos_ssh_host):
    for host in [ubuntu_ssh_host, centos_ssh_host]:
        with suspend_capture:
            with host.connect() as c:
                assert cmd.system.ssh_authorized_keys_list(c) == []
                cmd.system.ssh_authorized_keys_add(c, KEY)
                keys = cmd.system.ssh_authorized_keys_list(c)
                assert len(keys) == 1
                assert KEY in keys

                # Check not added second time
                cmd.system.ssh_authorized_keys_add(c, KEY)
                keys = cmd.system.ssh_authorized_keys_list(c)
                assert len(keys) == 1
                assert KEY in keys

                cmd.cli.run(c, "rm ~/.ssh/authorized_keys", hide=True)


@pytest.mark.remote
def test_ssh_authorized_keys_ensure(suspend_capture, ubuntu_ssh_host, centos_ssh_host):
    for host in [ubuntu_ssh_host, centos_ssh_host]:
        with suspend_capture:
            with host.connect() as c:
                assert cmd.system.ssh_authorized_keys_list(c) == []
                cmd.system.ssh_authorized_keys_ensure(c, [KEY, ])
                keys = cmd.system.ssh_authorized_keys_list(c)
                assert len(keys) == 1
                assert KEY in keys

                # Check not added second time
                cmd.system.ssh_authorized_keys_ensure(c, [KEY, ])
                keys = cmd.system.ssh_authorized_keys_list(c)
                assert len(keys) == 1
                assert KEY in keys

                cmd.cli.run(c, "rm ~/.ssh/authorized_keys", hide=True)


@pytest.mark.remote
def test_get_current_user_name(suspend_capture, ubuntu_ssh_host, centos_ssh_host):
    for host in [ubuntu_ssh_host, centos_ssh_host]:
        with suspend_capture:
            with host.connect() as c:
                assert cmd.system.get_current_user_name(c) == 'root'


@pytest.mark.remote
def test_get_current_user_id(suspend_capture, ubuntu_ssh_host, centos_ssh_host):
    for host in [ubuntu_ssh_host, centos_ssh_host]:
        with suspend_capture:
            with host.connect() as c:
                assert cmd.system.get_current_user_id(c) == 0


@pytest.mark.remote
def test_is_current_user_root(suspend_capture, ubuntu_ssh_host, centos_ssh_host):
    for host in [ubuntu_ssh_host, centos_ssh_host]:
        with suspend_capture:
            with host.connect() as c:
                assert cmd.system.is_current_user_root(c) is True
